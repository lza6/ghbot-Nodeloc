...
import { parseFinalConfirmation, describeConflictResolutionFailure } from "../src/review/conflictResolver.js";
...
test("failure descriptions are actionable and never expose raw command output", () => {
  const timeout = describeConflictResolutionFailure(new Error("initial goose conflict-editing pass timed out."));
  ...
  const push = describeConflictResolutionFailure(
    new Error("! [rejected] ... (stale info)")
  );
